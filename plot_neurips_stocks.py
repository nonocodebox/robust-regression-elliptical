...
class StocksDataset1(LabeledDataset):
    ...
    def _load_stocks_data(self):
        cache = 'stocks-nocrisis-new.pickle'
        train_start_date = date(2004, 1, 1)
        crisis_start_date = date(2007, 7, 1)
        crises_end_date = date(2009, 7, 1)
        ...
        if os.path.isfile(cache):
            ...
            train_dates = set(d for d in set(dates) if train_start_date <= d <= train_end_date and (d < crisis_start_date or d > crises_end_date))
    ...
    def _shuffle_data(self):
        ...
        for k in range(self.divisions):
            ...
            for j in range(self.shuffling):
                ...
                K = K_structure[stock_order, :][:, stock_order]
                print('K=', K)
...
class StocksDataset(LabeledDataset):
    ...
    def _shuffle_data(self):
        ...
        stocks_test = self.stock_values_test.T
        self.Es = []
        self.data = [[{'train': None, 'test': None, 'sigma': None} for _ in range(self.divisions * self.shuffling)] for _ in self.Ns]
        ...
        #K_structure = self._generate_structure_K(stocks_train)
        #K_structure = self._generate_structure_K((stocks_train - np.mean(stocks_train, axis=1, keepdims=True)) / np.std(stocks_train, axis=1, keepdims=True))
        ...
        for k in range(self.divisions):
            ...
            #print('glasso', division_train[:, 0])
            #print('glasso2', ((division_train - mu) / sigma)[:, 0])
...
def main():
    ...
    parser.add_argument('-s', '--shuffling', type=int, default=2)#default=100)
